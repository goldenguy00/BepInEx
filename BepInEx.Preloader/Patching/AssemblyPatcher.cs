...
namespace BepInEx.Preloader.Patching
{
    ...
    public static class AssemblyPatcher
    {
        ...
        public static void AddPatchersFromDirectory(string directory)
        {
            ...
            EnsureOldFPTSDoNotLoad(patchers);
        ...
        private static void CleanUpOldRoR2Bep()
        {
            try
            {
                // from patchers folder
                var oldPath = Path.Combine(Paths.PluginPath, "RoR2BepInEx");
        ...
        private static void EnsureOldFPTSDoNotLoad(Dictionary<string, List<PatcherPlugin>> patchers)
        {
            var dllFullPath = patchers.Keys.FirstOrDefault(assemblyPath =>
                assemblyPath.Contains("FixPluginTypesSerialization.dll") &&
                !assemblyPath.Contains("RiskofThunder-FixPluginTypesSerialization")
            );
            if (dllFullPath != null && patchers.Remove(dllFullPath))
            {
                Logger.LogInfo($"Removed Pre devotion update FPTS dll from the list of patchers to load");
            }
        }
